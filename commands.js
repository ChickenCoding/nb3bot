...
function regCommands(commandManager) {
    ...
    [
        ...
        new Command('background', ['bg', 'background', 'backgrounds'], 1, [], [],
            ...
            function (utils) {
                ...
                if (args.length > 0 && (bgUrl = checkIfSpecify()) !== null) {
                    utils.bot.sendChat(targetName + ' ' + bgUrl + "'s BGs: " + bgLinks[bgUrl]);
                }
                else {
        ...
        new Command('hate', ['hate'], 1, [], [],
            ...
            function (utils) {
                ...
                if (thatUser) {
                    ...
                    var love = utils.redisManager.decLove(utils.getUserId());
        ...
        new Command('love', ['love'], 1, [], [],
            ...
            function (utils) {
                ...
                if (thatUser) {
                    ...
                    // Ok everything should be good from here
                    var love = utils.redisManager.incLove(utils.getUserId());

                    utils.bot.sendChat('@' + thatUser.username + ' ' + utils.getUserUsername() + ' has sent some love your way! <3 You now have ' + love + ' hearts.');
                }
                else {
        ...
        new Command('selfpromotion', ['selfpromotion'], 1, [], [],
             ...
             */
            function (utils) {
                utils.bot.sendChat(utils.getUserUsername() + ' Please refrain from any self promotion in this room. As told in the rules: http://i.imgur.com/2zE0SPf.png');
            }
        )
        ,
        new Command('english', ['english'], 1, [], [],
             ...
             */
            function (utils) {
                utils.bot.sendChat(utils.getUserUsername() + ' Please stick to English in this room, doing otherwise will result in a mute.');
            }
        )
        ,
        new Command('shush', ['shush'], 1, [], [],
             ...
             */
            function (utils) {
                utils.bot.sendChat(utils.getUserUsername() + ' (Click for better quality) http://i.imgur.com/uFE8PfA.png');
        ...
        new Command('gema', ['gema', 'fuckgema', 'gemasucks', 'gemaisshit'], 1, [], [],
             ...
             */
            function (utils) {
                utils.bot.sendChat(utils.getUserUsername() + ' To bypass GEMA blocked videos you can use this extension http://www.unblocker.yt/en/');
            }
        )
        ,
        new Command('videocheck', ['videocheck'], 1, [], [],
             ...
             */
            function (utils) {
                utils.bot.sendChat(utils.getUserUsername() + ' Check if current video is available on any country at https://nb3x.nl/videocheck.php');
            }
        )
        ,
        ...
        new Command('roominfo', ['roominfo', 'community', 'room', 'info'], 1, [], [],
             ...
             */
            function (utils) {
                utils.bot.sendChat(utils.getUserUsername() + 'This community plays EDM | Trap | and Chill. Songs over 6:30 will be skipped so please follow the guidelines! Rules: http://git.io/vWJnY');
            }
        )
        ,
        ...
        new Command('commands', ['commands'], 0, [], [],
            ...
            function (utils) {
                ...
                commandManager.getCommandList().forEach(function(commandListElement) {
                    message += '!' + commandListElement.commandId + ' ';
                });
