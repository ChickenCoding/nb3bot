...
var globleCooldown = (process.env.COOLDOWN == undefined ? 30 : process.env.COOLDOWN); // Cooldown in seconds
 ...
 * @param {Command} command
 */
function calulateCooldown(utils, command) {
    if (command.cooldown <= 0) {
        return 0;
    }
    var coolD = command.cooldown;
    if (command.cooldown <= 1) {
        // if the cooldown is less than one take part of the default cooldown
        coolD = Math.floor(globleCooldown * command.cooldown);
    }
...
CommandManager.prototype.processCommand = function(utils) {
    ...
    if (!commandListElement.action) {
        ...
        utils.redisManager.getUserCommandCooldown(utils.getUserId(), function(onCooldown) {
            ...
            utils.redisManager.setUserCommandCooldown(utils.getUserId(), calulateCooldown(utils, command));
            runCommand(utils, command);
        });
    }
};
